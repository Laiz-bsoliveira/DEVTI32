...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // IMAGEM
    if(isset($_FILES['imagem']) && $_FILES['imagem']['error'] === UPLOD_ERR_OK) {
        $imagem = $_FILES['imagem']['tmp_name'];
        $imagem = file_get_contents($imagem_temp);
        $imagem_base64 = base64_encode($imagens);
    }
    ...
    if($retorno == 1){
        ...
        $sqlcadastra = "INSERT INTO catalogo (CAT_NOME, CAT_DESCRICAO, CAT_PRECO, CAT_TEMPO, CAT_ATIVO, CAT_IMAGEM )
...
<body>
    <div class="global">
        ...
        <div class="formulario">
            ...
            <form class='login' action="produto_cadastra.php" method="post">
            
                <label>NOME DO PRODUTO</label>
                <input type='text' name='txtnome' placeholder='Digite o nome completo' required>
                <br>
                <label>DESCRIÇÃO</label>
                <input type='textarea' name='txtdescricao'  maxlength='100' required>
                <br>
                <label>PREÇO</label>
